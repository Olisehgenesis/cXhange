...
const contractAddress = process.env.CONTRACT_ADDRESS_V2!;
...
if (!contractAddress || !mentoTokenBroker || !baseToken || !celoscanApiKey) {
  ...
  if (!contractAddress) {
    throw new Error('CONTRACT_ADDRESS_V2 is not set');
  }
  if (!mentoTokenBroker) {
...
async function main() {
  ...
  console.log('Verifying cXchangev2 contract on Alfajores...');
