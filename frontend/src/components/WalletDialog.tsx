...
export default function WalletDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const networkKey = chain?.id === 44787 ? 'alfajores' : 'mainnet';
  const assets = MENTO_ASSETS[networkKey];
          ...
          {isConnected && address ? (
            <>
              ...
              <div className="space-y-2">
                {assets.map(asset => {
                  const { data, isLoading } = useBalance({ address, token: asset.address, chainId: chain?.id });
                  return (
